...
<p align="center">
  ...
  <img src="https://raw.githubusercontent.com/colbymchenry/codegraph/main/assets/languages/edk2.svg?v=1" width="104" height="104" alt="EDK2" />
</p>
...
| **20+ Languages** | TypeScript, JavaScript, ArkTS, Python, Go, Rust, Java, C#, VB.NET, PHP, Ruby, C, C++, CUDA, Objective-C, Metal, Swift, Kotlin, Scala, Dart, Lua, Luau, R, Nix, Erlang, CFML, COBOL, Solidity, Terraform/OpenTofu, EDK2/UEFI, Svelte, Vue, Astro, Liquid, Pascal/Delphi |
...
| EDK2 / UEFI | `.inf`, `.dsc`, `.fdf`, `.dec`, `.uni`, `.vfr` | Full support (INF module definitions, DEC package declarations, DSC platform builds, FDF flash layouts, UNI localization strings, VFR HII forms; bidirectional C↔descriptor cross-resolution — INF ENTRY_POINT/CONSTRUCTOR/UNLOAD → C function, INF `[Protocols]`/`[Guids]`/`[Ppis]` → DEC declarations, C `PcdGet*`/`PcdSet*`/`FixedPcdGet*` → DEC PCD declarations, DSC `[LibraryClasses]` → library INF, DSC `[Components]` → INF, FDF `INF` lines → INF, library-class → DEC, DEPEX GUID dependencies; `EFIAPI`/`IN`/`OUT`/`OPTIONAL` C-dialect qualifiers blanked pre-parse so tree-sitter sees the real function signature) |
